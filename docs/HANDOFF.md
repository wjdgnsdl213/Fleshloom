...
소유자 결정으로 게임을 실시간 3D로 전환했다. 근거와 경계는 `docs/DECISIONS.md`
D-027, D-028에 있다.

### 현재 상태

기본 렌더러는 여전히 **`pixi`** 다. 3D는 `?renderer=three` 뒤에 있고,
`src/app/rendererSelection.ts`의 `DEFAULT_RENDERER`는 소유자가 아침 검토 후
직접 전환한다. 그 전까지 `main`은 매 커밋 배포 가능한 상태로 유지된다.

| 마일스톤 | 내용 | 커밋 |
|---|---|---|
| M1-M2 | 구조물 충돌 (원-vs-OBB, 5개 지점 + 스폰 유효성) | `d2e43d6`, `dd04c06` |
| M3 | 렌더러 시임 (`RendererHost`) | `6d3df68` |
| M4 | Three.js 백엔드 골격 + 카메라 리그 | `d7dff12` |
| M5 | 관절 보행 크리처 (팔 흔들기, 다리 교차) | `34c50f4` |
| M6 | 나머지 5종 리그 + 전투 스탠스 | `64f6c80` |
| M7 | 살아있는 테더 + 0.82초 포획 비트 | `f7595fd` |
| M8 | Warden 방사형 리그 | `5f97ec8` |
| M9 | 비 + 방역등 컬링 + 프레임 프로브 | `f5399fe` |

### 아침 검토 방법

1. `npm run dev` 후 `?renderer=three`로 실제 플레이. 같은 URL에서
   `?renderer=pixi`로 바꿔 바로 비교할 수 있다.
2. 나란히 비교 캡처는 `references/qa/2026-08-13-p8-<씬>-<렌더러>.png`에 있다.
   9개 QA 씬 x 2개 백엔드.
3. 3D를 기본으로 올릴 판단이 서면 `DEFAULT_RENDERER`만 `'three'`로 바꾸면 된다.
   그 외 코드 변경은 필요 없다.

### 3D가 아직 2D에 못 미치는 지점 (정직한 잔여 목록)

- 지면 텍스처가 없다. 아스팔트가 단색이라 2D의 노면 디테일이 사라졌다.
- 웅덩이가 배우를 반사하지 않는다. 광택 하이라이트만 있다.
- 아키타입별 색 분리가 없다. 실루엣과 비율로만 구분된다.
- Warden 공격 텔레그래프 지오메트리가 3D에 없다.
- 보행 미학은 기계가 판정할 수 없다. 수학적 건전성(테스트)과 실제 움직임
  (픽셀 diff)까지만 보장했고, 걸음걸이가 "좋아 보이는지"는 사람이 봐야 한다.

### 잠긴 규칙은 그대로다

방향 입력은 항상 사냥꾼을 움직이고, 고리 그리는 중에도 몸은 멈추지 않는다.
Toggle이 기본 입력, Hold는 규칙이 동일한 접근성 대안이다. 살아있는 고리만이
유일한 치명 수단이다. 새 각인은 자동으로 덮어쓰지 않는다. XP 변이는 런 영구다.
Fourfold Hunt는 후반 Apex다. 3D 백엔드는 규칙을 읽지도 바꾸지도 않는다 ??
스탠스는 시뮬레이션이 이미 발행하는 행동 문자열의 순수 함수다.
